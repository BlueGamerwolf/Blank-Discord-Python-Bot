...
# vc_id -> {"owner": user_id, "text": channel_id}
TEMP_VCS = {}
...
def setup(bot: commands.Bot):
    ...
    async def vc_voice_state_update(member, before, after):
        ...
        if after.channel and after.channel.name == HUB_CHANNEL_NAME:
            ...
            TEMP_VCS[vc.id] = {"owner": member.id, "text": tc.id}
            ...
            if len(before.channel.members) == 0:
                ...
                if isinstance(tc, discord.TextChannel):
                    ...
                    await post_owner_transcript(
                        ...
                        transcript_type="vc",
                        owner_id=data["owner"],
                        title=f"vc-{before.channel.id}-{closed_at.strftime('%Y%m%d-%H%M%S')}",
                        header_lines=[
    ...
    async def rename(ctx, *, name):
        ...
        if not vc:
            return await ctx.send("❌ You are not the VC owner.")
        ...
        await ctx.send(f"✅ Renamed to **{safe}**")
    ...
    async def limit(ctx, amount: int):
        ...
        await ctx.send(f"✅ Limit set to **{amount}**")
    ...
    async def block(ctx, member: discord.Member):
        ...
        if member.voice and member.voice.channel == vc:
            await member.move_to(None)
        await ctx.send(f"⛔ Blocked **{member.display_name}**")
    ...
    async def unblock(ctx, member: discord.Member):
        ...
        await ctx.send(f"✅ Unblocked **{member.display_name}**")
    ...
    async def invite(ctx, member: discord.Member):
        ...
        if not vc:
            return await ctx.send("❌ You are not the VC owner.")
        try:
            await member.send(f"🎧 You were invited to **{vc.name}**")

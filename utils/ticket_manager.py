...
def set_transcript_forum_id(guild_id: int, transcript_type: str, forum_id: int) -> None:
    data = _load_settings()
    guild_key = str(guild_id)
    guild_settings = data.get(guild_key, {})
    ...
    transcript_forums = guild_settings.get("transcript_forum_ids", {})
    if not isinstance(transcript_forums, dict):
        transcript_forums = {}

    transcript_forums[transcript_type] = forum_id
    guild_settings["transcript_forum_ids"] = transcript_forums
    data[guild_key] = guild_settings
    _save_settings(data)


def get_transcript_forum_id(guild_id: int, transcript_type: str) -> int | None:
    data = _load_settings()
    guild_settings = data.get(str(guild_id), {})
    ...
    forum_id = transcript_forums.get(transcript_type)
    return forum_id if isinstance(forum_id, int) else None
